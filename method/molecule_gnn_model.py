...
class GNN(nn.Module):
    def __init__(self, num_layer = 3, emb_dim = 128, JK="last", drop_ratio=0.1, gnn_type="gin"):
        if num_layer < 2:
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                ...
                self.gnns.append(RGCNConv(emb_dim, emb_dim, num_relations=65))
    ...
    def forward(self, *argv):
        ...
        for layer in range(self.num_layer):
            ...
            h_list.append(h)
        #!!!
        # h_stack = torch.stack(h_list, dim=0)  # shape: [num_layer+1, B, D]
        # weight = F.softmax(torch.nn.Parameter(torch.ones(4)) , dim=0).to(h_stack.device)  # shape: [num_layer + 1]
        # h_out = torch.sum(weight[:, None, None] * h_stack, dim=0)  # shape: [B, D]
