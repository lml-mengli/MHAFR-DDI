...
def get_3d_pos(pos, batch_batch):
    ...
    atom_dist_add = scatter_add(dim=0, index=edge_index[1], src=dist,dim_size=pos.size(0))
    ...
    for x in range(batch_node[len(batch_node)-1] + 1):
        ...
        if batch_node.count(x) <= 50:
            ...
            dist_zero = torch.zeros((50, 50), device=device)
            dist_t = dist_zero.index_put([row_t, col_t], dist_t)
...
def get_transformer_emb(batch, emb_2d, emb_3d, in_degree_2d, out_degree_2d, dist_3d, dist_m,spe, model_T_2d, model_T_3d):
    ...
    attention_scores_2d = torch.mean(attention_scores_2d, dim=-2) # every node bs*4*50
    attention_scores_2d = torch.mean(attention_scores_2d, dim=-2) # every head bs*50
    ...
    return emb_2d_high, emb_3d_high, attention_scores_2d, attention_scores_3d #emb_2d_high,attention_scores_2d
...
def get_mask_indices(attention_scores, x, batch_node): # x: batch.x ; batch_node: record the number of each graph's node
    ...
    # mask atom
    x_c[mask_list,0] = torch.tensor(119).to(device)
    x_c[mask_list,1] = torch.tensor(5).to(device)
    return x_c
...
class CL_model(nn.Module):
    ...
    def forward(self, batch_nol):
        ...
        attention_scores_1d = torch.mean(attention_scores_1d, dim=-2)
        attention_scores_1d = torch.mean(attention_scores_1d, dim=-2)
        ...
        temp = (emb_2d_low_nol_out, emb_3d_low_nol_out, emb_2d_high_nol, emb_3d_high_nol, emb_2d_low_mask_out,
                emb_3d_low_mask_out, emb_2d_high_mask, emb_3d_high_mask, emb_1d_low_out, emb_1d_high_out,
                emb_1d_low_mask_out, emb_1d_high_mask_out)
...
if __name__ == '__main__':
    ...
    # pretraing dataset 
    dataset = Drug3Dataset()
    # split_idx = dataset.get_idx_split()
    idx = len(dataset)

    randperm = torch.randperm(idx)
    train_idxs = randperm[: int((0.8) * idx)]
    dev_idxs = randperm[int(0.8 * idx): ]
    ...
    # model
    cl_model = CL_model()
    cl_model = torch_geometric.nn.DataParallel(cl_model.to(device), device_ids=device_ids)
